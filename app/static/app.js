...
function render(listings) {
  ...
  const avg = listings.length ? listings.reduce((sum,item)=>sum+item.price_kzt,0)/listings.length : 0;
  ...
  document.querySelector('#averagePrice').textContent = avg ? `${number.format(avg/1_000_000)} млн` : '—';
